...
import time
from .dfsmamba import dfsmamba, configs
from .dfsmamba.vision_mamba import SS2D
import torch.nn.functional as F
from .dfsmamba.utils import GenPatchEmbed2D
import torch.nn as nn
from .dfsmamba.configs import *

config = get_train_gen_config()
...
class SSSM_L1Loss(nn.Module):
    ...
    def __init__(self, lambda_weight=1.0, reduction='mean', aggregate_fn='mean', device='cuda:0'):
        ...
        # 初始化模块
        self.patch_embed = GenPatchEmbed2D(config).to(device)
        self.ssm = SS2D(
            d_model=config.mamba.embed_dims[0],
    ...
    def aggregate_states(self, states):
        ...
        if isinstance(states, torch.Tensor):
            ...
            if self.aggregate_fn == 'mean':
                return torch.mean(torch.stack(states, dim=0), dim=0)
            elif self.aggregate_fn == 'max':
                return torch.max(torch.stack(states, dim=0), dim=0)[0]
    ...
    def forward(self, real_B, fake_B, **kwargs):
        # 转换单通道为3通道
        if fake_B.shape[1] == 1:
        ...
        pred = self.patch_embed(fake_B)
        target = self.patch_embed(real_B)
        ...
        # Compute L1 loss
        loss = F.l1_loss(pred_aggregated, target_aggregated, reduction=self.reduction)

        # Scale by lambda_weight
        return self.lambda_weight * loss


class DFSMamba(BaseModel):
    def name(self):
        return 'DFSMamba'
    ...
    def initialize(self, opt):
        ...
        self.netG = dfsmamba.DFSMambaGenerator(gen_config).cuda(self.gpu_ids[0])
        if self.isTrain:
            use_sigmoid = opt.no_lsgan
            self.netD = dfsmamba.DFSMambaDiscriminator(disc_config).cuda(self.gpu_ids[0])
        ...
        if self.isTrain:
            ...
            self.criterionSSSM = SSSM_L1Loss(
                lambda_weight=opt.lambda_sssm if hasattr(opt, 'lambda_sssm') else 1.0,
                reduction='mean',
                aggregate_fn='mean',
                device=device
            )
    ...
    def backward_G(self):
        ...
        with torch.autograd.set_detect_anomaly(True):
            ...
            self.loss_G_SSSM = self.criterionSSSM(self.real_B, self.fake_B)


            self.loss_G = self.loss_G_GAN + self.loss_G_L1 + self.loss_ssim + self.loss_G_SSSM
